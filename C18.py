# -*- coding: utf-8 -*-
"""
ERA5 bucket-based CC3D event extraction — C18-strict connectivity
=================================================================

目的：
    将原来的 3D 26-neighborhood 事件追踪改为 C18-strict：
    - face + edge connectivity
    - 去除 3D corner connectivity
    - 仍然允许 t 到 t+1 的相邻连接
    - 降低 diagonal spatiotemporal merging

重要说明：
    在 3D array (time, y, x) 中：
        connectivity = 26:
            face + edge + corner 全部连接
            包括 dt=1, dy=1, dx=1 的三维角点连接
        connectivity = 18:
            face + edge 连接
            排除三维角点连接
            即排除 dt、dy、dx 三个方向同时变化的连接
        connectivity = 6:
            只允许 face connection，最严格

本脚本输出到新目录：
    OUT_ROOT/events_cc3d_C18_strict_face_edge_no_corner
...
# 关键修改 1：输出到新文件夹，避免覆盖原来的 26-neighborhood 结果
EVENTS_ROOT = OUT_ROOT / "events_cc3d_C18_strict_face_edge_no_corner"
...
# 关键修改 2：C18-strict：face + edge，去除 corner connectivity
CONNECTIVITY = 18
CONNECTIVITY_NAME = "C18_strict_face_edge_no_corner"
...
def main():
    ...
    print("[INFO] Definition        : 3D face + edge connectivity; corner connectivity removed")
